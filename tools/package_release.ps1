...
if (!(Test-Path -LiteralPath $Rar)) { throw 'WinRAR command-line packager is required' }
$nativeFiles = @(
...
foreach ($binary in @((Join-Path $NativeOutput 'KharvoxLayer.dll'),(Join-Path $LauncherOutput 'KharvoxLauncher.exe'))) {
    if ((Get-Item -LiteralPath $binary).VersionInfo.FileVersion -ne '0.5.0.320') { throw "Incorrect file version: $binary" }
}
$layer = Get-Content -Raw -LiteralPath (Join-Path $NativeOutput 'KharvoxLayer.json') | ConvertFrom-Json
if ($layer.layer.implementation_version -ne '320') { throw 'Incorrect layer manifest version' }
...
@("Release: $release", "Source commit: $SourceCommit", 'Renderers: AER; Native Stereo Experimental', 'Default backend: AER', '0.5 Beta: approved HUD/hand defaults, VR intro and bundled haptics integrations') | Set-Content -Encoding utf8 -LiteralPath (Join-Path $evidence 'SOURCE_REVISION.txt')
...
@('0.5 Beta package assembly verified', 'Renderers: AER; Native Stereo Experimental', 'Launcher self-test: passed (exit 0)', 'Layer/launcher file version: 0.5.0.320; manifest: 320', 'All explicitly required runtime files present; hashes in SHA256SUMS.txt', 'bHaptics SDK and bridge + PSVR2 Toolkit loader and bridge: included; x64 and approved DLL hashes verified', "Source commit: $SourceCommit") | Set-Content -Encoding utf8 -LiteralPath (Join-Path $evidence 'PACKAGE-VALIDATION.txt')
...
try {
    & $Rar a -r -ep1 -idq $archive $release
    if ($LASTEXITCODE -ne 0) { throw "RAR creation failed: $LASTEXITCODE" }
    & $Rar t -idq $archive
    if ($LASTEXITCODE -ne 0) { throw "RAR integrity test failed: $LASTEXITCODE" }
} finally { Pop-Location }
$archiveHash = (Get-FileHash -Algorithm SHA256 -LiteralPath $archive).Hash.ToLowerInvariant()
"$archiveHash  $release.rar" | Set-Content -Encoding ascii -LiteralPath ($archive + '.sha256')
