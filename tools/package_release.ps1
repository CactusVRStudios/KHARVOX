...
foreach ($binary in @((Join-Path $NativeOutput 'KharvoxLayer.dll'),(Join-Path $LauncherOutput 'KharvoxLauncher.exe'))) {
    if ((Get-Item -LiteralPath $binary).VersionInfo.FileVersion -ne '0.7.0.322') { throw "Incorrect file version: $binary" }
}
$layer = Get-Content -Raw -LiteralPath (Join-Path $NativeOutput 'KharvoxLayer.json') | ConvertFrom-Json
if ($layer.layer.implementation_version -ne '322') { throw 'Incorrect layer manifest version' }
...
@("Release: $release", "Source commit: $SourceCommit", 'Renderers: AER; Native Stereo Experimental', 'Default backend: AER', '0.7 Beta: approved HUD/hand defaults, VR intro and bundled haptics integrations') | Set-Content -Encoding utf8 -LiteralPath (Join-Path $evidence 'SOURCE_REVISION.txt')
...
@('0.7 Beta package assembly verified', 'Renderers: AER; Native Stereo Experimental', 'Launcher self-test: passed (exit 0)', 'Layer/launcher file version: 0.7.0.322; manifest: 322', 'All explicitly required runtime files present; hashes in SHA256SUMS.txt', 'bHaptics SDK and bridge + PSVR2 Toolkit loader and bridge: included; x64 and approved DLL hashes verified', "Source commit: $SourceCommit") | Set-Content -Encoding utf8 -LiteralPath (Join-Path $evidence 'PACKAGE-VALIDATION.txt')
