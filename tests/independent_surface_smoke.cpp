...
int main(){try{
    ...
    VkApplicationInfo application{VK_STRUCTURE_TYPE_APPLICATION_INFO};application.pApplicationName="KHARVOX independent presentation smoke";application.apiVersion=VK_API_VERSION_1_1;
    ...
    VkInstanceCreateInfo create{VK_STRUCTURE_TYPE_INSTANCE_CREATE_INFO};create.pApplicationInfo=&application;create.enabledExtensionCount=4;create.ppEnabledExtensionNames=extensions;
...
#define INSTANCE(name) auto name=reinterpret_cast<PFN_##name>(gipa(instance,#name));if(!name)throw std::runtime_error(#name)
    INSTANCE(vkDestroyInstance);INSTANCE(vkEnumeratePhysicalDevices);INSTANCE(vkGetPhysicalDeviceQueueFamilyProperties);INSTANCE(vkGetPhysicalDeviceSurfaceSupportKHR);INSTANCE(vkGetPhysicalDeviceSurfaceCapabilities2KHR);INSTANCE(vkCreateWin32SurfaceKHR);INSTANCE(vkDestroySurfaceKHR);INSTANCE(vkGetPhysicalDeviceSurfaceFormatsKHR);INSTANCE(vkCreateDevice);INSTANCE(vkGetDeviceProcAddr);
    WNDCLASSW windowClass{};windowClass.lpfnWndProc=DefWindowProcW;windowClass.hInstance=GetModuleHandleW(nullptr);windowClass.lpszClassName=L"KharvoxIndependentSurfaceSmoke";RegisterClassW(&windowClass);
    HWND window=CreateWindowW(windowClass.lpszClassName,L"KHARVOX presentation test",WS_POPUP|WS_VISIBLE,0,0,640,360,nullptr,nullptr,windowClass.hInstance,nullptr);
    ...
    VkPhysicalDeviceSwapchainMaintenance1FeaturesEXT maintenance{VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_SWAPCHAIN_MAINTENANCE_1_FEATURES_EXT};maintenance.swapchainMaintenance1=VK_TRUE;
    const char* deviceExtensions[]={VK_KHR_SWAPCHAIN_EXTENSION_NAME,VK_EXT_SWAPCHAIN_MAINTENANCE_1_EXTENSION_NAME};
    VkDeviceCreateInfo deviceInfo{VK_STRUCTURE_TYPE_DEVICE_CREATE_INFO};deviceInfo.pNext=&maintenance;deviceInfo.queueCreateInfoCount=1;deviceInfo.pQueueCreateInfos=&queueInfo;deviceInfo.enabledExtensionCount=2;deviceInfo.ppEnabledExtensionNames=deviceExtensions;
...
#define DEVICE(name) auto name=reinterpret_cast<PFN_##name>(vkGetDeviceProcAddr(device,#name));if(!name)throw std::runtime_error(#name)
    ...
    VkSurfacePresentModeEXT mode{VK_STRUCTURE_TYPE_SURFACE_PRESENT_MODE_EXT};mode.presentMode=VK_PRESENT_MODE_IMMEDIATE_KHR;
    VkPhysicalDeviceSurfaceInfo2KHR query{VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_SURFACE_INFO_2_KHR};query.surface=surface;query.pNext=&mode;
    VkSurfacePresentScalingCapabilitiesEXT scaling{VK_STRUCTURE_TYPE_SURFACE_PRESENT_SCALING_CAPABILITIES_EXT};VkSurfaceCapabilities2KHR caps{VK_STRUCTURE_TYPE_SURFACE_CAPABILITIES_2_KHR};caps.pNext=&scaling;
    CALL(vkGetPhysicalDeviceSurfaceCapabilities2KHR,physical,&query,&caps);
    const auto source=kharvox::independentSourceExtent(3060,3264,1.f,16384);
    if(!kharvox::supportsIndependentExtent(scaling,source))throw std::runtime_error("Scaling unsupported");
    ...
    VkSwapchainPresentScalingCreateInfoEXT scaleInfo{VK_STRUCTURE_TYPE_SWAPCHAIN_PRESENT_SCALING_CREATE_INFO_EXT};scaleInfo.scalingBehavior=VK_PRESENT_SCALING_STRETCH_BIT_EXT;
    VkSwapchainCreateInfoKHR swapInfo{VK_STRUCTURE_TYPE_SWAPCHAIN_CREATE_INFO_KHR};swapInfo.pNext=&scaleInfo;swapInfo.surface=surface;swapInfo.minImageCount=caps.surfaceCapabilities.minImageCount;swapInfo.imageFormat=formats[0].format;swapInfo.imageColorSpace=formats[0].colorSpace;swapInfo.imageExtent=source;swapInfo.imageArrayLayers=1;swapInfo.imageUsage=VK_IMAGE_USAGE_TRANSFER_DST_BIT;swapInfo.imageSharingMode=VK_SHARING_MODE_EXCLUSIVE;swapInfo.preTransform=caps.surfaceCapabilities.currentTransform;swapInfo.compositeAlpha=VK_COMPOSITE_ALPHA_OPAQUE_BIT_KHR;swapInfo.presentMode=mode.presentMode;swapInfo.clipped=VK_TRUE;
    ...
    VkFenceCreateInfo fenceInfo{VK_STRUCTURE_TYPE_FENCE_CREATE_INFO};VkFence presentFence{};CALL(vkCreateFence,device,&fenceInfo,nullptr,&presentFence);
    for(auto size:{VkExtent2D{640,360},VkExtent2D{1280,720},VkExtent2D{1920,1080},VkExtent2D{2560,1440}}){
        ...
        for(int frame=0;frame<3;++frame){
            ...
            VkSwapchainPresentFenceInfoEXT completion{VK_STRUCTURE_TYPE_SWAPCHAIN_PRESENT_FENCE_INFO_EXT};completion.swapchainCount=1;completion.pFences=&presentFence;
            VkPresentInfoKHR present{VK_STRUCTURE_TYPE_PRESENT_INFO_KHR};present.pNext=&completion;present.waitSemaphoreCount=1;present.pWaitSemaphores=&ready;present.swapchainCount=1;present.pSwapchains=&swapchain;present.pImageIndices=&index;presentCheck(vkQueuePresentKHR(queue,&present),"present");
            CALL(vkWaitForFences,device,1,&presentFence,VK_TRUE,10000000000ull);CALL(vkQueueWaitIdle,queue);CALL(vkResetFences,device,1,&presentFence);
        }
