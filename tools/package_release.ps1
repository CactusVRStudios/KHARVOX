...
foreach ($binary in @((Join-Path $NativeOutput 'KharvoxLayer.dll'),(Join-Path $LauncherOutput 'KharvoxLauncher.exe'))) {
    if ((Get-Item -LiteralPath $binary).VersionInfo.FileVersion -ne '0.8.0.327') { throw "Incorrect file version: $binary" }
}
$layer = Get-Content -Raw -LiteralPath (Join-Path $NativeOutput 'KharvoxLayer.json') | ConvertFrom-Json
if ($layer.layer.implementation_version -ne '327') { throw 'Incorrect layer manifest version' }
...
@('0.8 Beta package assembly verified', 'Renderers: AER; Native Stereo Experimental', 'Launcher self-test: passed (exit 0)', 'Layer/launcher file version: 0.8.0.327; manifest: 327', 'All explicitly required runtime files present; hashes in SHA256SUMS.txt', 'bHaptics SDK and bridge + PSVR2 Toolkit loader and bridge: included; x64 and approved DLL hashes verified', "Source commit: $SourceCommit") | Set-Content -Encoding utf8 -LiteralPath (Join-Path $evidence 'PACKAGE-VALIDATION.txt')
