...
foreach ($binary in @((Join-Path $NativeOutput 'KharvoxLayer.dll'),(Join-Path $LauncherOutput 'KharvoxLauncher.exe'))) {
    if ((Get-Item -LiteralPath $binary).VersionInfo.FileVersion -ne '0.9.0.328') { throw "Incorrect file version: $binary" }
}
$layer = Get-Content -Raw -LiteralPath (Join-Path $NativeOutput 'KharvoxLayer.json') | ConvertFrom-Json
if ($layer.layer.implementation_version -ne '328') { throw 'Incorrect layer manifest version' }
...
@("Release: $release", "Source commit: $SourceCommit", 'Local beta test build; not published', 'Renderers: AER; Native Stereo Experimental', 'Default backend: AER', '0.9 Beta: bounded virtual-texture page append; pending source requests retained for native residency retry', 'Approved 0.7 HUD/hand defaults, AMD fixes, VR intro and bundled integrations retained') | Set-Content -Encoding utf8 -LiteralPath (Join-Path $evidence 'SOURCE_REVISION.txt')
...
@('0.9 Beta package assembly verified', 'Renderers: AER; Native Stereo Experimental', 'Launcher self-test: passed (exit 0)', 'Layer/launcher file version: 0.9.0.328; manifest: 328', 'All explicitly required runtime files present; hashes in SHA256SUMS.txt', 'bHaptics SDK and bridge + PSVR2 Toolkit loader and bridge: included; x64 and approved DLL hashes verified', "Source commit: $SourceCommit") | Set-Content -Encoding utf8 -LiteralPath (Join-Path $evidence 'PACKAGE-VALIDATION.txt')
