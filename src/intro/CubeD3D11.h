...
inline HRESULT createEyeTarget(ID3D11Device* device, ID3D11Texture2D* texture,
    ...
    // OpenXR may return typeless backing textures. The view must use the
    // concrete format negotiated with xrCreateSwapchain, not infer the type.
...
class CubeRenderer {
    ...
    UINT capacity{},count{};
    static void check(HRESULT hr){if(FAILED(hr))throw std::runtime_error("Intro GPU renderer failed: "+std::to_string(unsigned(hr)));}
public:
...
float4 psMain(Output input):SV_Target {
    ...
    // Upload once per stereo frame. Both eyes reuse the same compact geometry.
    ...
    void drawEye(ID3D11DeviceContext* context, ID3D11RenderTargetView* target,
        ...
        // Do not retain a runtime-owned swapchain image after release.
